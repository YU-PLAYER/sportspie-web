...
const ModifyProfile = () => {
  ...
  const [isProfileImageVisible, setIsProfileImageVisible] = useState(true);
  const [isUserInfoVisible, setIsUserInfoVisible] = useState(true);
  const [isStatusMessageVisible, setIsStatusMessageVisible] = useState(true);
  const [isRecordVisible, setIsRecordVisible] = useState(true);

  const ProfileUpdate = async () => {
    try {
      const response = await axios.put('http://115.85.182.229:8080/', {
        profileImage,
        userInfo,
        statusMessage,
        Forward,
  ...
  const fetchUserData = async () => {
    try {
      const response = await axios.get('http://115.85.182.229:8080/');
      const { profileImage, userInfo, statusMessage, Forward, Midfielder, Defender, Goalkeeper} = response.data;
      ...
      console.error("업데이트에 실패하였습니다. : 12", error);
    }
  };
